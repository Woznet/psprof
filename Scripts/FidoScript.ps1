...
.GUID 584fa6de-f040-4397-a49b-9d40b430986c
...
.TAGS Windows, UEFI, ISO, PowerShell, Script, Download

.LICENSEURI https://github.com/pbatard/Fido/blob/master/LICENSE.txt

.PROJECTURI https://github.com/pbatard/Fido/
...
.RELEASENOTES
    ...
    - Version 1.0.0
        - Initial release of FidoScript.ps1
...
.DESCRIPTION
    A PowerShell download script for Microsoft Windows and UEFI Shell ISOs.
...
function GetElementById([object]$Request, [string]$Id) {
    try {
        ...
        return $Request.AllElements | Where-Object { $_.id -eq $Id }
    }
}
...
if ($Cmd) {
    ...
    if ($winver -le 6.1) {
        Get-Error(Get-Translation('This feature is not available on this platform.'))
        exit 403
    }
